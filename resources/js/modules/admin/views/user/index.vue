            ...
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{ t('users.title') }}</li>
                    </ol>
                </nav>
            </div>
        </div>
            ...
            <div class="col-xl-12">
                <div class="card custom-card">
                    <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-3 py-3">
                        <div class="d-flex flex-wrap align-items-center gap-1 users-toolbar-filters">
                            <div class="input-group input-group-sm users-toolbar-search">
                                ...
                                <button
                                    v-if="search"
                                    type="button"
                                    class="btn btn-light border users-search-clear"
                                    :title="t('users.clear_search')"
                                    @click="clearSearch"
                                >
                            ...
                            <button
                                v-if="showAllFilter"
                                type="button"
                                class="btn btn-sm users-filter-btn"
                                :class="statusFilter === 'all' ? 'users-filter-btn--all' : 'users-filter-btn--all-idle'"
                                @click="setStatusFilter('all')"
                            >
                                {{ t('users.filter_all') }} ({{ counts.total }})
                            </button>
                            <button
                                type="button"
                                class="btn btn-sm users-filter-btn"
                                :class="statusFilter === 'active' ? 'users-filter-btn--active' : 'users-filter-btn--active-idle'"
                                @click="setStatusFilter('active')"
                            >
                                {{ t('users.filter_active') }} ({{ counts.active }})
                            </button>
                            <button
                                type="button"
                                class="btn btn-sm users-filter-btn"
                                :class="statusFilter === 'inactive' ? 'users-filter-btn--inactive' : 'users-filter-btn--inactive-idle'"
                                @click="setStatusFilter('inactive')"
                            >
                                {{ t('users.filter_inactive') }} ({{ counts.inactive }})
                            </button>
                            <button
                                type="button"
                                class="btn btn-sm users-filter-btn"
                                :class="statusFilter === 'blocked' ? 'users-filter-btn--blocked' : 'users-filter-btn--blocked-idle'"
                                @click="setStatusFilter('blocked')"
                            >
                                {{ t('users.filter_blocked') }} ({{ counts.blocked }})
                            </button>
                        </div>
                                        ...
                                        >
                                            <td class="ps-4">
                                                <input
                                                    ...
                                                    @change="onRowSelect(user.id, $event.target.checked)"
                                                >
                                            </td>
                                            <td>
                                                <div class="d-flex align-items-center gap-2">
                                                    <span class="avatar avatar-sm avatar-rounded">
                                                        <img
                                                            v-if="user.avatar_thumb || user.avatar"
                                            ...
                                            <td>
                                                ...
                                                <span v-if="user.updated_at" class="d-block text-muted fs-11">
                    ...
                    <div v-if="pagination && !loading" class="card-footer border-top-0">
                        <div class="d-flex align-items-center flex-wrap gap-3">
                            ...
                            <div class="d-flex align-items-center gap-2 ms-md-auto">
                                <label class="text-muted fs-13 mb-0" for="users-per-page">{{ t('users.per_page') }}</label>
                                <select
                                    id="users-per-page"
                            ...
                            <nav aria-label="Users pagination" class="pagination-style-4">
...
const counts = computed(() => ({
    total: usersStore.total ?? pagination.value?.total ?? 0,
    active: usersStore.activeCount ?? 0,
    inactive: usersStore.inactiveCount ?? 0,
    blocked: usersStore.blockedCount ?? 0,
}));
...
const allSelected = computed(() => {
    ...
    return users.value.every((user) => selectedIds.value.includes(Number(user.id)));
});
...
const entriesLabel = computed(() => {
    ...
    return t('users.showing_entries', {
...
function statusSelectClass(status) {
    if (status === 'active') {
        return 'users-status-select--active';
    }

    if (status === 'blocked') {
        return 'users-status-select--blocked';
    }
...
function confirmDeleteSelected() {
    deleteConfirm.open({
        title: t('users.delete_selected_title'),
        message: t('users.confirm_delete_selected'),
...
async function handleDeleteConfirm() {
    ...
    try {
        if (deleteConfirm.state.payload?.type === 'multiple') {
            ...
            await deleteUser(deleteConfirm.state.payload.id);
        }
    } finally {
...
.users-filter-btn--all {
    background-color: #845adf;
    border-color: #845adf;
    color: #fff;
}

.users-filter-btn--all-idle {
...
.users-filter-btn--active-idle {
    background-color: rgba(38, 191, 148, 0.12);
    border-color: rgba(38, 191, 148, 0.35);
    color: #26bf94;
}

.users-filter-btn--inactive {
    background-color: #6c757d;
    border-color: #6c757d;
    color: #fff;
}

.users-filter-btn--inactive-idle {
    background-color: #f3f6f8;
    border-color: #dee2e6;
    color: #6c757d;
}

.users-filter-btn--blocked {
    background-color: #e6533c;
    border-color: #e6533c;
    color: #fff;
}

.users-filter-btn--blocked-idle {
...
.users-status-select--active {
    background-color: rgba(38, 191, 148, 0.12);
    border-color: rgba(38, 191, 148, 0.35);
    color: #26bf94;
}

.users-status-select--inactive {
    background-color: #f3f6f8;
    border-color: #dee2e6;
    color: #6c757d;
}

.users-status-select--blocked {
    background-color: rgba(230, 83, 60, 0.12);
    border-color: rgba(230, 83, 60, 0.35);
    color: #e6533c;
}
