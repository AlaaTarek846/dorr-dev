                            ...
                            <table class="table text-nowrap table-striped table-hover mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th v-if="showActionsColumn" scope="col" class="text-end pe-4">{{ t('countries.actions') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    >
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                <div>
                                                    <button
                                                        v-if="canUpdate && !isTrashedRecord(country)"
                                                        type="button"
                                                    ...
                                                    <span v-else class="fw-semibold text-default">{{ displayTranslatedName(country, locale) }}</span>
                                        ...
                                        <td>
                                            <span v-if="isTrashedRecord(country)" class="badge bg-danger-transparent">
                                            ...
                                            <span v-else class="badge" :class="country.status ? 'bg-success-transparent' : 'bg-secondary-transparent'">
                                                {{ country.status ? t('countries.filter_active') : t('countries.filter_inactive') }}
                                            </span>
                                        </td>
                                        <td>
                                            <span class="d-block">{{ formatCatalogDate(catalogPrimaryDate(country), locale) }}</span>
                                            <span v-if="catalogShowUpdatedSubtext(country)" class="d-block text-muted fs-11">
                                                {{ t('countries.updated') }}: {{ formatCatalogDate(country.updated_at, locale) }}
                                            </span>
                                            <span v-if="catalogShowDeletedSubtext(country, isDeletedView)" class="d-block text-muted fs-11">
                                                {{ t('catalog.deleted_at') }}: {{ formatCatalogDate(country.deleted_at, locale) }}
                                            </span>
                                        </td>
                                        <td v-if="showActionsColumn" class="text-end pe-4">
                                            <div v-if="isTrashedRecord(country)" class="btn-list justify-content-end">
                                                <button
                                                    v-if="canUpdate"
                                                    type="button"
                                                    class="btn btn-sm btn-success-light btn-icon"
                                                    :title="t('catalog.restore_title')"
                                                    @click="confirmRestore(country.id)"
                                                >
                                                ...
                                                <button
                                                    ...
                                                    @click="confirmForceDelete(country.id)"
                                                >
                                                    <i class="ri-delete-bin-7-line"></i>
                                                </button>
                                            </div>
                                            <div v-else-if="!isTrashedRecord(country)" class="btn-list justify-content-end">
                                                <button
                                                    v-if="canUpdate"
                                                    type="button"
...
const {
    ...
    canMultipleDelete,
    showActionsColumn,
} = useCatalogPermissions('countries');
...
const counts = computed(() => ({
    ...
    deleted: countriesStore.deletedCount ?? 0,
}));
...
const {
    ...
    restoreItem: restoreCountry,
    forceDeleteItem: forceDeleteCountry,
    forceDeleteSelected,
