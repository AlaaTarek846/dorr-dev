            ...
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{ t('currencies.title') }}</li>
                    </ol>
                </nav>
            </div>
        </div>
                        ...
                        <div class="d-flex flex-wrap align-items-center gap-1 catalog-toolbar-filters">
                            <div class="input-group input-group-sm catalog-toolbar-search">
                                ...
                                <button
                                    ...
                                    :title="t('currencies.clear_search')"
                                    @click="clearSearch"
                                >
                                    ...
                                    >
                                        <td v-if="canMultipleDelete" class="ps-4">
                                            <input
                                                ...
                                                @change="onRowSelect(currency.id, $event.target.checked)"
                                            >
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <span class="badge bg-primary-transparent">{{ currency.code }}</span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            <div
                                                ...
                                                :class="{
                                                    on: currency.status,
                                                    'catalog-status-toggle--loading': isTogglingStatus(currency.id),
                                                }"
                                                role="button"
                                                tabindex="0"
                                                :aria-busy="isTogglingStatus(currency.id)"
                                                @click="toggleStatus(currency)"
                                                @keydown.enter.space.prevent="toggleStatus(currency)"
                                            >
                                                <span></span>
                                            </div>
                                        ...
                                        <td v-if="showActionsColumn" class="text-end pe-4">
                                            ...
                                            <div v-else-if="!isTrashedRecord(currency)" class="btn-list justify-content-end">
                                                ...
                                                <button
                                                    ...
                                                    @click="confirmDelete(currency.id)"
                                                >
                    ...
                    <div v-if="pagination && !loading" class="card-footer border-top-0">
                        <div class="d-flex align-items-center flex-wrap gap-3">
                            ...
                            <div class="d-flex align-items-center gap-2 ms-md-auto">
                                <label class="text-muted fs-13 mb-0" for="currencies-per-page">{{ t('currencies.per_page') }}</label>
                                <select
                                    id="currencies-per-page"
                            ...
                            <nav aria-label="Currencies pagination" class="pagination-style-4">
...
const allSelected = computed(() => {
    ...
    return currencies.value.every((currency) => selectedIds.value.includes(Number(currency.id)));
});
...
const entriesLabel = computed(() => {
    ...
    return t('currencies.showing_entries', {
...
function displayName(currency) {
    const translation = currency.translations?.find((item) => item.locale === locale.value);

    return translation?.name || currency.name || '-';
}
...
function formatExchangeRate(currency) {
    ...
    return Number(currency.exchange_rate).toLocaleString(locale.value === 'ar' ? 'ar-EG' : 'en-US', {
