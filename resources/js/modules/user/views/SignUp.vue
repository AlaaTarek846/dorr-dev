        ...
        <form @submit.prevent="submit">
            <div class="row gy-3">
                ...
                <div class="col-xl-12 text-center">
                    ...
                    <router-link :to="{ name: 'user.login' }" class="text-primary ms-1">
                        {{ t('sign_in') }}
                    </router-link>
                </div>
            </div>
...
async function submit() {
    ...
    try {
        const { data } = await userAxios.post('/api/user/v1/register', {
            name: form.name.trim(),
        ...
        sessionStorage.setItem(
            `user_otp_resend_until:${data.data.flow_token}`,
