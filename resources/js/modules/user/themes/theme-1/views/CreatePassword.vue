<template>
    <div>
        ...
        <p class="h5 fw-semibold mb-2">{{ t('user_auth.create_password_title') }}</p>
        <p class="mb-4 text-muted op-7 fw-normal">{{ t('user_auth.create_password_subtitle') }}</p>
        ...
        <form @submit.prevent="submit">
            <div class="row gy-3">
                ...
                <div class="col-xl-12">
                    <AuthFormInput
                        input-id="user-create-password-confirmation"
                ...
                <div class="col-xl-12 d-grid mt-2">
                    <button type="submit" class="btn btn-lg btn-primary" :disabled="loading">
                        {{ loading ? t('user_auth.saving_password') : t('user_auth.save_password') }}
                    </button>
                </div>
            </div>
        </form>
...
async function submit() {
    ...
    try {
        const { data } = await userAxios.post('/api/user/v1/create-password', {
        ...
        if (error.response?.status === 422) {
            ...
            errors.general = error.response?.data?.message ?? t('user_auth.create_password_failed');
        }
    } finally {
        loading.value = false;
    }
